...
public class CourseRepositoryJDBCTemplateImpl implements CourseRepository {
    ...
    //language=SQL
    private static final String SELECT_FIND_BY_NAME = "select * from course where name = ?";
    ...
    public Optional<Course> findByName(String name) {
        Course course;
        try {
            course = jdbcTemplate.query(SELECT_FIND_BY_NAME, courseResultSetExtractor, name);
        } catch (EmptyResultDataAccessException e) {
            return Optional.empty();
        }
        assert course != null;
        course.setStudentList(jdbcTemplate.query(SELECT_ALL_STUDENTS,
                studentRowMapper, course.getId()));
        course.setTeacherList(jdbcTemplate.query(SELECT_ALL_TEACHER,
                teacherRowMapper, course.getId()));
        return Optional.of(course);
